...
async function main() {
  try {
    ...
    if (userBalance < parseUnits("300", 6)) {
      throw new Error("Không đủ USDC balance (cần ít nhất 300 USDC)");
    }
    const vaultParams = {
      agentName: "TestPoolTwoStrategies",
      asset: usdcAddress,
      tokenName: "Test Vault Two",
      tokenSymbol: "TV2",
    ...
    //Thêm 2 strategies (A rồi B, queue: A, B)
    ...
    //Deposit 300 USDC (tự động vào A do autoAllocate)
    const depositAmount = parseUnits("300", 6);
    console.log("Phê duyệt USDC cho Vault (300 USDC)...");
    await usdc.approve(newVaultAddress, depositAmount, { gasLimit: 1_000_000 });
    console.log("Thực hiện deposit 300 USDC...");
    await vault.deposit(depositAmount, deployer, { gasLimit: 2_000_000 });
    console.log("Deposit hoàn tất");
    await printVaultState(vault, newVaultAddress);
    await printStrategyInfo(vault, strategyA, "A - Morpho Gauntlet");
    await printStrategyInfo(vault, strategyB, "B - Morpho Seamless");
    //Chờ 30 giây để có gain/loss
    console.log("Chờ 30 giây để farm gain/loss...");
    await delay(30);
    //ProcessReport cho A để farm gain
    ...
    await printVaultState(vault, newVaultAddress);
    await printStrategyInfo(vault, strategyA, "A - Morpho Gauntlet");
    //Phân bổ debt đều (150 USDC mỗi strategy)
    const halfDebt = parseUnits("150", 6);
    console.log("Cập nhật debt cho A thành 150 USDC...");
    ...
    console.log("Cập nhật debt cho B thành 150 USDC...");
    ...
    await printStrategyInfo(vault, strategyA, "A - Morpho Gauntlet");
    await printStrategyInfo(vault, strategyB, "B - Morpho Seamless");
    //ProcessReport cho A và B để farm gain
    console.log("Gọi processReport cho Strategy A...");
    ...
    // Kiểm tra currentDebt của A trước khi rút
    const strategyAData = await vault.strategies(strategyA);
    if (strategyAData.currentDebt > 0) {
      ...
      console.log("Strategy A không có debt, bỏ qua rút debt.");
    }

    console.log("Thêm debt vào B (set target=300)...");
    txUpdateB = await vault.updateDebt(strategyB, parseUnits("300", 6), 0, { gasLimit: 2_000_000 });
    ...
    // Bước 12: ProcessReport cho B để farm gain sau rebalance
